...
def main():
    ...
    with tab2:
        ...
        n_steps = st.slider("Simulation steps", 5, 50, 20)

        if st.button("▶ Start live simulation"):
            placeholder = st.empty()
            prob_history = []
            replay = sensor_replay(df, equipment)

            for step in range(n_steps):
                try:
                    live_features = next(replay)
                except StopIteration:
                    break

                prob, risk = predict(model, live_features)
                issues     = diagnose(live_features)
                action     = recommend(risk, issues)
                prob_history.append(prob)

                with placeholder.container():
                    st.markdown(f"**Step {step + 1}/{n_steps}** — {equipment}")
                    c1, c2, c3 = st.columns(3)
                    c1.metric("Failure prob.", f"{prob:.1%}")
                    c2.metric("Risk",          risk_badge(risk))
                    c3.metric("Priority",      f"${priority_score(prob, equipment, risk):,.0f}")
                    ...
                    if len(prob_history) > 1:
                        ...
                        ax.plot(prob_history, color="#E25A1C", linewidth=1.5)
                        ax.axhline(0.7, color="red",    linestyle="--", linewidth=0.8, label="High threshold")
                        ax.axhline(0.4, color="orange", linestyle="--", linewidth=0.8, label="Medium threshold")
                        ax.set_xlabel("Step")
    ...
    with tab5:
        ...
        X_sample = get_training_data(df, equipment).sample(1, random_state=1)
        explainer  = shap.TreeExplainer(base_clf)
        shap_vals  = explainer.shap_values(X_sample)

        fig3, _ = plt.subplots()
        shap.summary_plot(shap_vals[1], X_sample, show=False, plot_size=(8, 3))
        st.pyplot(fig3)
        plt.close(fig3)
